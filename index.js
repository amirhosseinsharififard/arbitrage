...
import {
  ...
  setWebInterface,
} from "./src/Arbitrage Logic/core/multiCurrencyManager.js";
...
} from "./src/Arbitrage Logic/arbitrage_bot/arbitrage.js";
...
} from "./src/Arbitrage Logic/services/index.js";
...
async function initializeSystem() {
  try {
    ...
    if (!authResult) {
      console.error(
        "❌ GitHub authentication failed! Application cannot start."
      );
    ...
    console.log(
      "✅ GitHub authentication passed, continuing system initialization..."
    );
    ...
    const performanceModule = await import(
      "./src/Arbitrage Logic/utils/performanceOptimizer.js"
    );
...
function displayTradingStatus() {
  ...
  console.log(
    `P&L: ${FormattingUtils.formatCurrency(
      status.totalProfit
    )} | Total trades: ${
      status.totalTrades
