...
class RequestManager extends EventEmitter {
    constructor() {
        ...
        // Request limits per exchange - OPTIMIZED for higher concurrency
        this.requestLimits = {
            mexc: pLimit(10), // 10 concurrent requests (increased from 3)
            lbank: pLimit(8), // 8 concurrent requests (increased from 2)
            kcex: pLimit(3), // 3 concurrent requests (increased from 1)
            ourbit: pLimit(3), // 3 concurrent requests (increased from 1)
            xt: pLimit(3), // 3 concurrent requests (increased from 1)
            dexscreener: pLimit(8) // 8 concurrent requests (increased from 2)
        };
        ...
        // Rate limiting settings - OPTIMIZED for higher throughput
        this.rateLimits = {
            mexc: { requests: 0, window: 60000, maxRequests: 200 }, // 200 requests per minute (increased from 100)
            lbank: { requests: 0, window: 60000, maxRequests: 100 }, // 100 requests per minute (increased from 30)
            kcex: { requests: 0, window: 60000, maxRequests: 60 }, // 60 requests per minute (increased from 20)
            ourbit: { requests: 0, window: 60000, maxRequests: 60 }, // 60 requests per minute (increased from 20)
            xt: { requests: 0, window: 60000, maxRequests: 60 }, // 60 requests per minute (increased from 20)
            dexscreener: { requests: 0, window: 60000, maxRequests: 150 } // 150 requests per minute (increased from 50)
        };

        // Start rate limit reset timer
        this.startRateLimitReset();

        console.log('🚀 Request Manager initialized with OPTIMIZED limits');
    }
    ...
    async executeRequest(exchangeId, requestFunction, requestId = null) {
        ...
        try {
            ...
            const startTime = activeRequest ? activeRequest.startTime : 0;
            this.requestHistory.set(requestKey, {
                exchangeId,
